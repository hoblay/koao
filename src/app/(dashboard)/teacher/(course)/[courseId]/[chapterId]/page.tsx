    ...
    <div className="px-24 py-8 border-t border-t-zinc-900"> 
      <div className="flex items-center justify-between border-b border-b-zinc-900 py-4 px-4">
        <div className="flex gap-3">
          ...
          <Link href={`/teacher/${params.courseId}/${params.chapterId}/upload`}><Tag  name="Adicionar aula" startContent={<IconBookUpload className="w-5 h-5 text-zinc-500 dark:text-zinc-400"/>}/></Link> 
        </div>
          ...
          <table className="w-full text-sm text-left rtl:text-right text-zinc-500 dark:text-zinc-400">
              ...
              <tbody>
                  {chapter.data.lessons.map((lesson, index) => (
                    ...
                    { lesson.video?.duration &&  formatSecondsToMinutes(lesson.video.duration)}
                    </td>
                    <td className="px-6 py-4">
                      { lesson.video?.sizeInBytes &&  formatBytes(lesson.video.sizeInBytes)}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex">
                        <Tag  name="Pronto "/>
