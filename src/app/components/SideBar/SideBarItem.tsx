    ...
    <>
      <li className={` mt-2 ${className}`}>
        <Link href={href || "#"} className={`flex items-center py-2 px-4 text-zinc-600 rounded-lg dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 group transition-all duration-75 ease-in-out ${open ? '': ''} ${!parent && !Icon? 'pl-9' : ''} ${isOpen && parent && open ? ' text-purple-500 dark:bg-purple-700 bg-purple-100 hover:bg-purple-200 dark:hover:bg-purple-800': ''}`} onClick={() => handleClick()}>
          ...
          {parent && (
            <ChevronDownIcon className={`ml-2 w-5 h-5  transition-all transform duration-300 ${isOpen ? "rotate-180" : ""}`}/>
          )}
        </Link>
        {parent &&  (
