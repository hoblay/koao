...
function Course({ className, progress, img, name, price, modules }:CourseProps) {
  ...
  return ( 
    <Link href="/course" className=''>
      <div className="relative flex w-full md:max-w-[20rem] max-w-max flex-col rounded-lg bg-zinc-50 dark:bg-zinc-900 hover:dark:bg-zinc-900/60 bg-clip-border text-zinc-700 dark:text-zinc-100 shadow-sm hover:shadow hover:cursor-pointer">
        <div
        ...
        <div className="px-4 py-4 flex-col  space-y-3">
          ...
          <div className={`w-full bg-zinc-200 rounded-md dark:bg-zinc-700 ${!progress && 'hidden'}`} >
            <div className={`bg-purple-600 dark:bg-purple-700 text-xs font-medium text-zinc-50 dark:text-zinc-100 text-center p-0.5 leading-none rounded-md ${pp}`} > {progress}%</div>
          </div>
